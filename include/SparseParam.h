...
class SparseParam : public BaseParam {
 public:
  n3ldg_cpu::Tensor2D aux_square;
  n3ldg_cpu::Tensor2D aux_mean;
  NRVec<bool> indexers;
  ...
  void value(const int &featId, n3ldg_cpu::Tensor1D &out) {
    assert(out.dim == val.row);
    memcpy(out.v, val[featId], val.row * sizeof(dtype));
  }

  void value(const vector<int> &featIds, n3ldg_cpu::Tensor1D &out) {
    assert(out.dim == val.row);
  ...
  void loss(const int &featId, const n3ldg_cpu::Tensor1D &loss) {
  ...
  void loss(const vector<int> &featIds, const n3ldg_cpu::Tensor1D &loss) {
    assert(loss.dim == val.row);
    int featNum = featIds.size();
    int featId;
    for (int i = 0; i < featNum; i++) {
